...
-- =============================================================================
-- Key formula: uuid5(NS_HASH, "service_enums:task_status.{pub_name}")
--
...
INSERT INTO [dbo].[service_enums]
  ([key_guid], [pub_enum_type], [pub_name], [pub_value], [pub_notes])
VALUES
  ('79357640-A35E-5B8C-843A-837083978C70', 'task_status', 'PENDING',    0,
   'Declared, awaiting pickup by the consuming module.'),

  ('EE3E40AD-7BC2-5C93-81EC-3822C8791F44', 'task_status', 'RUNNING',    1,
   'Picked up and actively executing.'),

  ('79128086-3678-5DDA-967D-8DA02D41A7E5', 'task_status', 'COMPLETE',   2,
   'Executed successfully.'),

  ('5D32C5A0-5D83-5C2B-95B7-54404C068704', 'task_status', 'FAILED',     3,
   'Execution failed. priv_error holds the failure detail.'),

  ('475DBD6E-C7E8-537D-8581-C8277D2F0698', 'task_status', 'CANCELLING', 4,
...
-- =============================================================================
-- Key formula: uuid5(NS_HASH, "service_enums:task_operation_ddl.{pub_name}")
--
...
INSERT INTO [dbo].[service_enums]
  ([key_guid], [pub_enum_type], [pub_name], [pub_value], [pub_notes])
VALUES
  ('8516211A-E77F-5BFA-9709-17EF0994EB2E', 'task_operation_ddl', 'CREATE_TABLE',    0,
   'Create a new table. pub_spec holds full table/column spec.'),

  ('CA757B99-5A75-58E2-AA45-0EACD9FED659', 'task_operation_ddl', 'ALTER_COLUMN',    1,
   'Alter an existing column. pub_spec holds table + column spec.'),

  ('C0352EBD-8E26-53D1-918F-E2F1EA07B175', 'task_operation_ddl', 'CREATE_INDEX',    2,
   'Create a new index. pub_spec holds index spec + column membership.'),

  ('592FEE5D-0EF9-581C-803B-F34CB7281D4F', 'task_operation_ddl', 'DROP_CONSTRAINT', 3,
   'Drop a constraint by name. pub_spec holds table + constraint name.'),

  ('32B301C0-3BAC-55BF-A773-8BCD1E6BAD10', 'task_operation_ddl', 'DROP_INDEX',      4,
...
-- =============================================================================
-- Key formula: uuid5(NS_HASH, "service_enums:task_disposition.{pub_name}")
--
...
INSERT INTO [dbo].[service_enums]
  ([key_guid], [pub_enum_type], [pub_name], [pub_value], [pub_notes])
VALUES
  ('5A5FC56F-C6D0-5DE0-94C5-2F20B19FCAD4', 'task_disposition', 'REVERSIBLE',   0,
   'Task effect can be undone via a compensating task (e.g., create_index → drop_index).'),

  ('C3B202EB-2405-5F75-A4B6-6899EF7812CB', 'task_disposition', 'IRREVERSIBLE', 1,
   'Task effect cannot be undone through the system (e.g., drop_table). Requires external recovery.'),

  ('531AADE2-47F7-5984-9875-3ACEAB414F1D', 'task_disposition', 'TRANSIENT',    2,
   'Task has no persistent effect; safe to discard mid-execution.'),

  ('0F54301D-3856-59BF-9E59-442DA2255A95', 'task_disposition', 'CANCELLABLE',  3,
